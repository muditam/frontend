               ...
               <Box sx={{ mt: 1 }}>
                 <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                   ...
                   {!!paymentLink && (
                     <Button
                       onClick={async () => {
                         const ok = await copyToClipboardSafely(paymentLink);
                         setCopyStatus(ok ? "Payment link copied." : "Copy blocked by browser permissions.");
                       }}
                     >
                       Copy Payment Link
                     </Button>
                 ...
                 {copyStatus ? (
                   <Typography variant="caption" sx={{ mt: 1, display: "block", color: "#5c6f82" }}>
                     {copyStatus}
                   </Typography>
                 ) : null}
