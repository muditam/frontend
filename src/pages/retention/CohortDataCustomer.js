...
function Timeline({ baseISO, span, blocks, products, starts }) {
 ...
 const BAR_COLOR = "linear-gradient(135deg, #DBEAFE 0%, #EAF2FF 100%)";
 const BAR_BORDER = "#A8C3E6";


 return (
   <Box sx={{ overflowX: "auto", borderRadius: 2, border: "1px solid #D4DFEC", bgcolor: "#F8FBFF", p: 1 }}>
     ...
     >
       <Box sx={{ fontWeight: 800, textAlign: "left", p: 1, color: "#0F172A" }}>Product</Box>
       <Box sx={{ fontWeight: 800, textAlign: "left", p: 1, color: "#0F172A" }}>Start Date</Box>
       {Array.from({ length: span }).map((_, i) => (
         <Box key={`mname-${i}`} sx={{ textAlign: "center", p: 1, fontWeight: 800, color: "#1E293B" }}>
     ...
     >
       ...
       <Box sx={{ p: 1 }} />
       {Array.from({ length: span }).map((_, i) => (
         <Box key={`midx-${i}`} sx={{ textAlign: "center", p: 1, color: "#64748B", fontWeight: 600 }}>{`M${i}`}</Box>
       ))}
     </Box>
     ...
     {products.map((p) => (
       ...
       >
         {/* Left columns */}
         <Box sx={{ fontWeight: 700, p: 1, color: "#0F172A" }}>{productAbbr(p)}</Box>
         <Box sx={{ p: 1, color: "#475569", fontWeight: 500 }}>
       ...
       <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
         ...
         <Stack direction="row" spacing={1}>
           <Chip
             ...
             sx={{ bgcolor: "#EAF0F8", border: "1px solid #D4DFEC", fontWeight: 700, color: "#1E293B" }}
           />
           <Chip
             size="small"
...
export default function CohortDataCustomer({
 ...
 return (
   <Box sx={{ p: 2, mt: 2, borderRadius: 3, bgcolor: "transparent" }}>
     {loading ? (
