...
function MessageTicks({ status }) {
  ...
  if (st === "delivered") {
    return (
      <DoneAllIcon
        sx={{ fontSize: 16, ml: 0.5, color: "rgba(0,0,0,0.55)", verticalAlign: "middle" }}
      />
    );
  }
...
export default function WhatsAppChatDrawer({
  ...
  // ✅ Socket ref + joined room tracking
  ...
  const fetchMessages = async () => {
    if (!phone10) return;
    const res = await axios.get(`${API_BASE}/api/whatsapp/messages`, {
  ...
  }, [open, phone10, leadId, currentUserName]);
  ...
  const doRephrase = async () => {
    ...
    try {
      ...
      // backend returns { success:true, result:"..." } in my suggested route
  ...
  return (
    ...
    >
      ...
      {/* Template Composer (same as before) */}
