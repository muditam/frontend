...
const PRESIGN_DOWN_API = "https://muditamleads-14f32a10d7f7.herokuapp.com/api/other-videos/presign-download";
...
function UploadFileDialog({ open, onClose, item, onUploaded, showSnack }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const urls = await uploadFilesToWasabi(
        selectedFiles,
        getAuthHeaders(),
        setProgress
      );
      ...
      const extra =
        urls.length > 1
          ? urls.map((u, i) => `File ${i + 1}: ${u.url}`).join("\n")
          : "";
      ...
      await axios.post(
        ...
        {
          cutVideoUrl: urls[0].url,
          cutVideoName: urls[0].originalName,
          cutComment: finalComment,
        },
      ...
      <DialogContent sx={{ pt: 3, px: 3, display: "flex", flexDirection: "column", gap: 3 }}>
        {item && (
          ...
          >
            <Stack direction="row" gap={1} mb={0.5}>
              <OtherVideoIdBadge id={item.otherVideoId} />
            ...
            >
              {previewRaw(item.scriptText, 180)}
            </Typography>
          </Box>
        )}
        ...
        >
          ...
          <Typography sx={{ fontSize: "0.8rem", color: "#94a3b8", mt: 0.5 }}>
            MP4, MOV, AVI, WebM, MKV · Multiple allowed
          </Typography>
    ...
    >
        ...
        {item && (
          ...
          >
            ...
            >
              {item.cutVideoUrl
                ? "✅ File uploaded"
                : "⚠️ No file uploaded yet — you can still mark done"}
            </Typography>
          </Box>
        )}
      </DialogContent>
      ...
      >
        ...
        <Button
          ...
          disabled={confirming}
          startIcon={
            confirming ? <CircularProgress size={15} color="inherit" /> : <CheckIcon sx={{ fontSize: 17 }} />
          }
          sx={{
            bgcolor: "#059669",
            boxShadow: "none",
...
// Main page
// ─────────────────────────────────────────────────────────────
export default function OtherVideoCutPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const pRows = shootDoneRes.data.otherVideos || [];
      const dRows = cutDoneRes.data.otherVideos || [];
    ...
    <Box sx={{ bgcolor: "#f8fafc", minHeight: "100vh", color: "#0f172a", p: 4 }}>
      <Box mb={3}>
        <Typography sx={{ fontWeight: 800, fontSize: "1.8rem", color: "#0f172a" }}>
          Other Video Cut Studio
        </Typography>
        <Typography sx={{ color: "#64748b", fontSize: "0.95rem", mt: 0.5 }}>
          Upload cut files · Mark items as Cut Done · Track uploader and comments
      ...
      <Paper sx={{ ...lightPaper, p: 2, mb: 2 }}>
        <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap" gap={1}>
          <TextField
            size="small"
            label="Search (idea / id / link / creator)"
                    ...
                    {pendingItems.length === 0 ? (
                      ...
                      pendingItems.map((s, i) => (
                        <TableRow key={s._id} sx={{ "&:hover td": { bgcolor: "#f8fafc" } }}>
                          ...
                          <TableCell sx={tdSx}>
                            <OtherVideoIdBadge id={s.otherVideoId} />
                          </TableCell>
                          ...
                          <TableCell sx={tdSx}>
                            {s.cutVideoUrl && s.cutVideoUrl !== "pending" ? (
                              <Stack direction="row" spacing={0.5} alignItems="center">
                                <Button
                                  size="small"
                                  onClick={() => openPlayer(s.cutVideoUrl, `${s.otherVideoId} — Cut`)}
                                ...
                                <Tooltip title="Download">
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      handleDownload(
                                        s.cutVideoUrl,
                                        s.cutVideoName || s.otherVideoId + "_cut"
                                      )
                                    }
                                    sx={{
                                      color: "#64748b",
          ...
          {viewContent && (
            <Box>
              <Stack direction="row" spacing={1.5} mb={2.5}>
                <OtherVideoIdBadge id={viewContent.otherVideoId} />
