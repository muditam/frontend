...
const PRESIGN_API = "https://muditamleads-14f32a10d7f7.herokuapp.com/api/other-videos/presign";
...
function ShootBadge({ hasShoot }) {
  return (
    ...
    >
      {hasShoot ? "Have a Shoot" : "Need Shoot"}
    </Box>
  );
}
...
function AssignCell({
  ...
  const [value, setValue] = useState(
    marketingEmployees.find((e) => e.fullName === item.editAssignedTo) || null
  );
  ...
  }, [item.editAssignedTo, marketingEmployees]);
        ...
        {item && (
          ...
          >
            <Stack direction="row" alignItems="center" gap={1} mb={0.5} flexWrap="wrap">
              <OtherVideoId id={item.otherVideoId} />
              <TypeBadge label={item.scriptType} />
              <ShootBadge hasShoot={item.hasShoot} />
            </Stack>
            ...
            >
              {previewText(item.scriptText, 220)}
            </Typography>

            {mode === "reupload" && item.editFileUrl && (
...
// Main page
// ─────────────────────────────────────────────────────────────
export default function OtherVideoEditPage() {
  ...
  const commonFilterParams = useMemo(() => {
    ...
    if (filters.otherVideoId) p.scriptId = filters.otherVideoId;
    if (filters.scriptType) p.scriptType = filters.scriptType;
    if (filters.creator) p.creator = filters.creator;
    if (filters.hasShoot !== "") p.hasShoot = filters.hasShoot;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPendingItems(pendingRes.data.otherVideos || []);
      setDoneItems(doneRes.data.otherVideos || []);

      setPendingTotal(
        pendingRes.data.pagination?.total ?? (pendingRes.data.otherVideos || []).length
      );
      setDoneTotal(
        doneRes.data.pagination?.total ?? (doneRes.data.otherVideos || []).length
      );
    } catch {
  ...
  const applyFilters = () => {
    setFilters({
      ...
      otherVideoId: (filtersDraft.otherVideoId || "").trim(),
      scriptType: filtersDraft.scriptType || "",
      creator: (filtersDraft.creator || "").trim(),
      hasShoot: filtersDraft.hasShoot,
    });
    setPendingPage(0);
    setDonePage(0);
  };
  ...
  return (
    <Box sx={{ bgcolor: "#f4f5f7", minHeight: "100vh", color: "#111827", p: 4 }}>
      <Box mb={3}>
        ...
        >
          Other Video <Box component="span" sx={{ color: "#4f46e5" }}>Edit Workspace</Box>
        </Typography>

        <Typography sx={{ color: "#6b7280", fontSize: "0.9rem", mt: 0.5 }}>
          Assign editor · Download cut / source file · Upload edited video · Track status
      ...
      <Paper sx={{ ...lightPaper, mb: 2, p: 2 }}>
        <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap" gap={1}>
          ...
          <TextField
            ...
            placeholder="e.g. OVD0001"
            value={filtersDraft.otherVideoId}
            onChange={(e) =>
              setFiltersDraft((s) => ({ ...s, otherVideoId: e.target.value }))
            }
          ...
          <FormControl size="small" sx={{ minWidth: 170, ...inputSx }}>
            ...
            <Select
              label="Have a Shoot"
              value={filtersDraft.hasShoot}
              onChange={(e) =>
                setFiltersDraft((s) => ({ ...s, hasShoot: e.target.value }))
              }
            >
              <MenuItem value="">All</MenuItem>
                        ...
                        <TableRow key={item._id} sx={rowSx}>
                          ...
                          <TableCell sx={{ ...tdSx, maxWidth: 220 }}>
                            <Stack direction="row" alignItems="center" spacing={0.5}>
                              ...
                              >
                                {previewText(item.scriptText, 160)}
                              </Typography>
                              ...
                              <Tooltip title="View Full Content">
                                <IconButton
                                  size="small"
                                  onClick={() => {
                                    setViewText(normalizeScriptText(item.scriptText));
                                    setViewOpen(true);
                                  }}
                                  sx={{
                                    color: "#6b7280",
                              ...
                              <Stack direction="row" spacing={0.5} alignItems="center">
                                <Button
                                  ...
                                  onClick={() =>
                                    openPlayer(
                                      item.cutVideoUrl,
                                      `${item.otherVideoId} — ${item.hasShoot ? "Source" : "Cut"}`
                                    )
                                  }
                                  ...
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      handleDownload(
                                        item.cutVideoUrl,
                                        item.cutVideoName ||
                                          `${item.otherVideoId}_${item.hasShoot ? "source" : "cut"}`
                                      )
                                    }
                                    sx={{
                                      color: "#6b7280",
                                  ...
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      setCommentModal({
                                        ...
                                        title: `Comment — ${item.otherVideoId}`,
                                      })
                                    }
                                    sx={{
                                      color: "#6b7280",
                          ...
                          <TableCell sx={tdSx}>
                            ...
                            >
                              {fmt(item.hasShoot ? item.proceedToShootAt : item.cutDoneAt)}
                            </Typography>
                          </TableCell>
                        ...
                        <TableRow key={item._id} sx={rowSx}>
                          ...
                          <TableCell sx={tdSx}>
                            <Stack
                              ...
                              gap={0.5}
                            >
                              {item.editFileUrl && item.editFileUrl !== "pending" ? (
                                <>
                                  <Button
                                    size="small"
                                    onClick={() =>
                                  ...
                                  <Tooltip title="Download">
                                    <IconButton
                                      ...
                                      onClick={() =>
                                        handleDownload(
                                          item.editFileUrl,
                                          item.editFileName || item.otherVideoId + "_edited"
                                        )
                                      }
                                      sx={{
                                        color: "#6b7280",
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  setThumbTarget(item);
                                  setThumbMode(item.editThumbUrl ? "reupload" : "upload");
                                  setThumbOpen(true);
                                }}
                                sx={{
                                  borderColor: "#fdba74",
                              ...
                              >
                                {item.editThumbUrl ? "Reupload Thumbnail" : "Upload Thumbnail"}
                              </Button>

                              {item.editThumbUrl && item.editThumbUrl !== "pending" && (
                                <Tooltip title="Download Thumbnail">
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      handleDownload(
                                        item.editThumbUrl,
                                        item.editThumbName || `${item.otherVideoId}_thumb`
                                      )
                                    }
                                    sx={{
                                      color: "#6b7280",
                                  ...
                                  <IconButton
                                    ...
                                    onClick={() =>
                                      setCommentModal({
                                        ...
                                        title: `Edit Comment — ${item.otherVideoId}`,
                                      })
                                    }
                                    sx={{
                                      color: "#6b7280",
      ...
      >
          ...
          {dialogTarget && (
            ...
            >
              <Stack direction="row" alignItems="center" gap={1} mb={1} flexWrap="wrap">
                ...
                <ShootBadge hasShoot={dialogTarget.hasShoot} />
                <Typography sx={{ fontSize: "0.8rem", color: "#6b7280" }}>
        ...
        >
          ...
          <Button
            ...
            disabled={saving}
            startIcon={
              saving ? <CircularProgress size={16} color="inherit" /> : <EditIcon />
            }
            sx={{
              bgcolor: "#ea580c",
              color: "#ffffff",
