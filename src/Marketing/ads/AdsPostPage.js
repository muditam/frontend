...
function HasShootPill({ hasShoot }) {
  return (
    <Box
      sx={{
        display: "inline-block",
        ...
        {item && (
          ...
          >
            <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap">
              <AdsNo id={item.adsVideoId} />
              <TypePill label={item.adType} />
              <HasShootPill hasShoot={!!item.hasShoot} />
            </Stack>
            <Typography sx={{ mt: 1, fontSize: "0.9rem", color: "#111827", fontWeight: 700 }}>
              {item.title}
            </Typography>
          </Box>
        )}
...
function InlineStatusSelect({ item, reload, toast, openReasonDlg }) {
  ...
  const allowedStatuses = item?.hasShoot
    ? POST_STATUSES
    : POST_STATUSES.filter((s) => s !== "Reshoot");
...
function EditedVideoBtn({ item, openPlayer, handleDownload }) {
  ...
  return (
    <Stack direction="row" spacing={0.5} alignItems="center" flexWrap="wrap">
      <Button
        size="small"
        onClick={() => openPlayer(url, `${item.adsVideoId} — Edited Video`)}
...
export default function AdsPostPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const eItems = eRes.data.adsVideos || [];
      ...
      toast("Failed to load ads videos", "error");
      setPendingList([]);
      setPublishedList([]);
      setPendingTotal(0);
      setPublishedTotal(0);
  ...
  const openReasonDlg = (item, status) => setReasonDlg({ open: true, item, status });
  const closeReasonDlg = () => setReasonDlg({ open: false, item: null, status: "" });
    ...
    <Box sx={{ bgcolor: "#f8fafc", minHeight: "100vh", color: "#0f172a", p: 4 }}>
      <Box mb={3}>
        ...
        <Typography sx={{ color: "#64748b", fontSize: "0.95rem", mt: 0.5 }}>
          Review final edited ads videos · Set content status · Track publishing.
        </Typography>
      </Box>
                    ...
                    {pendingList.length === 0 ? (
                      <TableRow>
                        ...
                        >
                          ...
                          />
                          No ads videos ready to post yet.
                        </TableCell>
                      </TableRow>
                    ) : (
          ...
          {viewContent && (
            <Box>
              <Stack direction="row" spacing={1.5} mb={2.5} alignItems="center" flexWrap="wrap">
                <AdsNo id={viewContent.adsVideoId} />
                <TypePill label={viewContent.adType} />
                <HasShootPill hasShoot={!!viewContent.hasShoot} />
              </Stack>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <Typography sx={{ fontSize: "1rem", color: "#111827", fontWeight: 700 }}>
                  {viewContent.title}
                </Typography>
              </Box>
