...
const inputSx    = { "& .MuiOutlinedInput-root": { bgcolor: "#ffffff", "&.Mui-focused fieldset": { borderColor: "#4f46e5" } }, "& .MuiInputLabel-root.Mui-focused": { color: "#4f46e5" } };
const acSx = {
  minWidth: 170,
  "& .MuiOutlinedInput-root": { bgcolor: "#ffffff", fontSize: "0.85rem", color: "#111827", py: "2px !important", "& fieldset": { borderColor: "#d1d5db" }, "&.Mui-focused fieldset": { borderColor: "#4f46e5" } },
  "& .MuiInputLabel-root": { color: "#6b7280", fontSize: "0.85rem" },
  "& .MuiInputLabel-root.Mui-focused": { color: "#4f46e5" },
};
const thSx = { color: "#4b5563", fontSize: "0.75rem", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", borderBottom: "1px solid #e5e7eb", bgcolor: "#f9fafb", py: 2, whiteSpace: "nowrap" };
...
function ScriptId({ id }) {
  return <Typography sx={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: "0.85rem", color: "#4f46e5", letterSpacing: "0.02em" }}>{id}</Typography>;
}
function TypeBadge({ label }) {
  return <Box sx={{ display: "inline-block", px: 1.2, py: 0.3, borderRadius: "100px", fontSize: "0.75rem", fontWeight: 500, bgcolor: "#eef2ff", color: "#4f46e5", border: "1px solid #c7d2fe", whiteSpace: "nowrap" }}>{label}</Box>;
}
function EditStatusChip({ status }) {
  ...
  return <Box sx={{ display: "inline-block", px: 1.2, py: 0.4, borderRadius: "100px", fontSize: "0.75rem", fontWeight: 700, bgcolor: c.bg, color: c.color, border: `1px solid ${c.border}`, whiteSpace: "nowrap" }}>{status}</Box>;
}
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { bgcolor: "#ffffff", borderRadius: 2 } }}>
      <DialogTitle sx={{ borderBottom: "1px solid #e5e7eb", py: 2, px: 3, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Stack direction="row" alignItems="center" gap={1}><PlayIcon sx={{ color: "#4f46e5", fontSize: 24 }} /><Typography sx={{ fontWeight: 700, color: "#111827", fontSize: "1.1rem" }}>{title || "File Preview"}</Typography></Stack>
        <IconButton size="small" onClick={onClose} sx={{ color: "#6b7280" }}><CloseIcon fontSize="small" /></IconButton>
      </DialogTitle>
      <DialogContent sx={{ pt: 3, pb: 3, px: 3 }}>
        {isVideo
          ...
          : <Box sx={{ textAlign: "center", py: 8, bgcolor: "#f8fafc", borderRadius: 2, border: "1px dashed #d1d5db" }}>
              ...
              <Button component="a" href={url} target="_blank" variant="contained" startIcon={<OpenInNewIcon />}
                sx={{ bgcolor: "#4f46e5", "&:hover": { bgcolor: "#4338ca" }, textTransform: "none", fontWeight: 600, borderRadius: 1.5, boxShadow: "none" }}>Open Externally</Button>
            </Box>
        }
      </DialogContent>
    </Dialog>
...
function CommentModal({ open, onClose, comment, title }) {
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx: { bgcolor: "#ffffff", borderRadius: 2 } }}>
      <DialogTitle sx={{ borderBottom: "1px solid #e5e7eb", py: 2, px: 3, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Stack direction="row" alignItems="center" gap={1}><CommentIcon sx={{ color: "#4f46e5", fontSize: 20 }} /><Typography sx={{ fontWeight: 700, fontSize: "1rem" }}>{title || "Comment"}</Typography></Stack>
        <IconButton size="small" onClick={onClose} sx={{ color: "#6b7280" }}><CloseIcon fontSize="small" /></IconButton>
      </DialogTitle>
      <DialogContent sx={{ pt: 2.5, pb: 3, px: 3 }}>
        {comment
          ? <Box sx={{ bgcolor: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: 2, p: 2 }}><Typography sx={{ fontSize: "0.9rem", color: "#374151", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{comment}</Typography></Box>
...
function AssignCell({ script, marketingEmployees, onAssigned, showSnack, canAssign }) {
  ...
  useEffect(() => { setValue(marketingEmployees.find((e) => e.fullName === script.editAssignedTo) || null); }, [script.editAssignedTo, marketingEmployees]);
  ...
  const handleChange = async (_, newVal) => {
    ...
    try {
      ...
      showSnack(`Assigned to ${newVal.fullName} ✅`); onAssigned();
    } catch (e) { showSnack(e.response?.data?.message || "Assignment failed", "error"); }
    finally { setSaving(false); }
  };


  if (!canAssign) return <Typography sx={{ fontSize: "0.85rem", color: value ? "#374151" : "#9ca3af", fontWeight: value ? 500 : 400 }}>{value ? value.fullName : "—"}</Typography>;


  return (
    <Stack direction="row" alignItems="center" spacing={1}>
      <Autocomplete size="small" options={marketingEmployees} getOptionLabel={(o) => o.fullName || ""} value={value} onChange={handleChange} sx={acSx}
        slotProps={{ paper: { sx: { bgcolor: "#ffffff", color: "#111827", border: "1px solid #e5e7eb", fontSize: "0.85rem", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.1)" } } }}
        renderOption={(props, option) => <Box component="li" {...props} sx={{ fontSize: "0.85rem", color: "#374151", "&:hover": { bgcolor: "#f3f4f6 !important" } }}>{option.fullName}</Box>}
...
function UploadVideoDialog({ open, onClose, script, onUploaded, showSnack }) {
  ...
  const reset      = () => { setSelectedFile(null); setUploadProgress(0); setLabel(""); setComment(""); setEditStatus("Done"); setHoldReason(""); };
  ...
  const handleUpload = async () => {
    ...
    setUploading(true); setUploadProgress(0); setLabel("Getting upload URL…");
    try {
      ...
      setLabel("Saving to database…"); setUploadProgress(100);
      await axios.post(`${API}/${script._id}/edit-upload`,
        { editFileUrl: url, editFileName: selectedFile.name, editComment: comment, editStatus, editHoldReason: holdReason },
        { headers: getAuthHeaders(), withCredentials: true });
      showSnack("Edited video uploaded! 🎬"); onUploaded(); handleClose();
    } catch (err) { showSnack(err.message || "Upload failed", "error"); }
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: "#ffffff", borderRadius: 2 } }}>
      <DialogTitle sx={{ color: "#111827", fontWeight: 700, borderBottom: "1px solid #e5e7eb", pb: 2, px: 3, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Stack direction="row" alignItems="center" gap={1}><UploadIcon sx={{ color: "#ea580c" }} /><Typography sx={{ fontSize: "1.1rem", fontWeight: 700 }}>Upload Edited Video</Typography></Stack>
        <IconButton size="small" onClick={handleClose} disabled={uploading} sx={{ color: "#6b7280" }}><CloseIcon fontSize="small" /></IconButton>
      </DialogTitle>
      <DialogContent sx={{ pt: 3, px: 3, display: "flex", flexDirection: "column", gap: 3 }}>
        {script && (
          <Box sx={{ bgcolor: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 1.5, p: 2 }}>
            <Stack direction="row" alignItems="center" gap={1} mb={0.5}><ScriptId id={script.scriptId} /><TypeBadge label={script.scriptType} /></Stack>
            <Typography sx={{ fontSize: "0.85rem", color: "#4b5563", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{script.scriptText}</Typography>
          </Box>
        )}
        <Box onClick={() => !uploading && fileInputRef.current?.click()}
          sx={{ border: "2px dashed #d1d5db", borderRadius: 2, p: 4, textAlign: "center", cursor: uploading ? "default" : "pointer", "&:hover": uploading ? {} : { borderColor: "#ea580c", bgcolor: "#fff7ed" } }}>
          ...
          {selectedFile
            ? <Box>
                ...
                <Button size="small" onClick={(e) => { e.stopPropagation(); setSelectedFile(null); }} sx={{ mt: 1, color: "#dc2626", textTransform: "none", fontSize: "0.8rem" }}>Remove</Button>
              </Box>
            : <>
          ...
          <input ref={fileInputRef} type="file" accept="video/*,.mp4,.mov,.avi,.webm,.mkv,.m4v"
            style={{ display: "none" }} onChange={(e) => { const f = e.target.files?.[0]; if (f) setSelectedFile(f); }} disabled={uploading} />
        </Box>
        ...
        {needsReason && <TextField label="Reason *" multiline minRows={2} value={holdReason} onChange={(e) => setHoldReason(e.target.value)} disabled={uploading} sx={inputSx} />}
        <TextField label="Comment (optional)" multiline minRows={2} placeholder="Notes for the team…" value={comment} onChange={(e) => setComment(e.target.value)} disabled={uploading} sx={inputSx} />
        {uploading && (
          <Box>
            ...
            {uploadProgress > 0 && uploadProgress < 100 && <Typography sx={{ fontSize: "0.75rem", color: "#9ca3af", mt: 0.5 }}>Uploading directly to storage — large files may take a few minutes</Typography>}
          </Box>
        )}
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 3, pt: 2, borderTop: "1px solid #e5e7eb", gap: 1 }}>
        <Button onClick={handleClose} disabled={uploading} sx={{ color: "#6b7280", textTransform: "none" }}>Cancel</Button>
...
export default function EditPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [editPendingRes, cutDoneRes, editDoneRes, empRes] = await Promise.all([
        ...
        axios.get(API, { params: cutDoneParams, headers, withCredentials: true }),
        axios.get(API, { params: doneParams,    headers, withCredentials: true }),
        axios.get(EMP_API, { params: { role: "Marketing" }, withCredentials: true }),
      ]);


      let pendingAll = [...(cutDoneRes.data.scripts || []), ...(editPendingRes.data.scripts || [])];
  ...
  const openDialog  = (s) => { setDialogTarget(s); setForm({ editStatus: s.editStatus || "", editHoldReason: s.editHoldReason || "", editComment: s.editComment || "" }); setErrors({}); setDialogOpen(true); };
  ...
  return (
    <Box sx={{ bgcolor: "#f4f5f7", minHeight: "100vh", color: "#111827", p: 4 }}>
      ...
      <Stack direction="row" spacing={2} mb={3} alignItems="center" flexWrap="wrap" gap={1}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)}
          sx={{
            ...
            bgcolor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: 2, px: 1, minHeight: 48, width: "fit-content", boxShadow: "0 1px 2px rgba(0,0,0,0.05)",
          }}>
          ...
          {tab === 0 && (
            <TableContainer component={Paper} sx={lightPaper}>
              <Table size="medium">
                <TableHead>
                  <TableRow>
                    {["#","Script ID","Type","Script Preview","Creator","Cut Video","Cut Comment","Cut Done At","Assigned To","Upload Edited Video",""].map((h) => <TableCell key={h} sx={thSx}>{h}</TableCell>)}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {pendingScripts.length === 0
                    ? <TableRow><TableCell colSpan={11} align="center" sx={{ py: 8, borderBottom: "none", color: "#6b7280" }}>
                        <MagicIcon sx={{ fontSize: 36, mb: 1, opacity: 0.3, display: "block", mx: "auto", color: "#9ca3af" }} />
                        {pendingDate.dateFrom || pendingDate.dateTo ? "No scripts for selected date range" : "No scripts awaiting edit"}
                      </TableCell></TableRow>
                      ...
                      <TableRow key={s._id} sx={rowSx}>
                        ...
                        <TableCell sx={{ ...tdSx, maxWidth: 200 }}>
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            <Typography sx={{ fontSize: "0.85rem", color: "#4b5563", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{s.scriptText}</Typography>
                            <Tooltip title="View Full Script"><IconButton size="small" onClick={() => { setViewScript(s); setViewOpen(true); }} sx={{ color: "#6b7280", "&:hover": { color: "#4f46e5", bgcolor: "#eef2ff" } }}><ViewIcon sx={{ fontSize: 16 }} /></IconButton></Tooltip>
                          </Stack>
                        </TableCell>
                        <TableCell sx={tdSx}><Typography sx={{ fontSize: "0.85rem", color: "#4b5563" }}>{s.createdBy}</Typography></TableCell>
                        <TableCell sx={tdSx}>
                          {s.cutVideoUrl && s.cutVideoUrl !== "pending"
                            ? <Stack direction="row" spacing={0.5} alignItems="center">
                                <Button size="small" onClick={() => openPlayer(s.cutVideoUrl, `${s.scriptId} — Cut`)} startIcon={<PlayIcon sx={{ fontSize: 16 }} />}
                                  sx={{ color: "#4f46e5", textTransform: "none", fontWeight: 600, fontSize: "0.8rem", p: "4px 8px", border: "1px solid #c7d2fe", borderRadius: 1.5, "&:hover": { bgcolor: "#eef2ff" } }}>Play</Button>
                                <Tooltip title="Download"><IconButton size="small" onClick={() => handleDownload(s.cutVideoUrl, s.cutVideoName || s.scriptId + "_cut")} sx={{ color: "#6b7280", "&:hover": { color: "#4f46e5", bgcolor: "#eef2ff" } }}><DownloadIcon sx={{ fontSize: 16 }} /></IconButton></Tooltip>
                              </Stack>
                        ...
                        <TableCell sx={{ ...tdSx, maxWidth: 140 }}>
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            <Typography sx={{ fontSize: "0.85rem", color: "#6b7280", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{s.cutComment || "—"}</Typography>
                            {s.cutComment && <Tooltip title="View full"><IconButton size="small" onClick={() => setCommentModal({ open: true, text: s.cutComment, title: `Cut Comment — ${s.scriptId}` })} sx={{ color: "#6b7280", flexShrink: 0, "&:hover": { color: "#4f46e5", bgcolor: "#eef2ff" } }}><ViewIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>}
                          </Stack>
                        </TableCell>
                        ...
                        <TableCell sx={tdSx}>
                          <Tooltip title="Update status / comment"><IconButton size="small" onClick={() => openDialog(s)} sx={{ color: "#6b7280", "&:hover": { color: "#ea580c", bgcolor: "#ffedd5" } }}><EditIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
                        </TableCell>
                      </TableRow>
                    ))
                  }
          ...
          {tab === 1 && (
            <TableContainer component={Paper} sx={lightPaper}>
              <Table size="medium">
                <TableHead>
                  <TableRow>
                    {["#","Script ID","Type","Script Preview","Assigned To","Edited Video","Edit Status","Comment","Done At","Done By",""].map((h) => <TableCell key={h} sx={thSx}>{h}</TableCell>)}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {doneScripts.length === 0
                    ? <TableRow><TableCell colSpan={11} align="center" sx={{ py: 8, borderBottom: "none", color: "#6b7280" }}>
                        <MagicIcon sx={{ fontSize: 36, mb: 1, opacity: 0.3, display: "block", mx: "auto", color: "#9ca3af" }} />
                        {doneDate.dateFrom || doneDate.dateTo ? "No scripts for selected date range" : "No edits completed yet"}
                      </TableCell></TableRow>
                    : doneScripts.map((s, i) => (
                      <TableRow key={s._id} sx={rowSx}>
                        <TableCell sx={{ ...tdSx, color: "#9ca3af", fontSize: "0.8rem" }}>{i + 1}</TableCell>
                        ...
                        <TableCell sx={tdSx}>
                          {s.editFileUrl && s.editFileUrl !== "pending"
                            ? <Stack direction="row" spacing={0.5} alignItems="center">
                                <Button size="small" onClick={() => openPlayer(s.editFileUrl, `${s.scriptId} — Edited`)} startIcon={<PlayIcon sx={{ fontSize: 16 }} />}
                                  sx={{ color: "#4f46e5", textTransform: "none", fontWeight: 600, fontSize: "0.8rem", p: "4px 8px", border: "1px solid #c7d2fe", borderRadius: 1.5, "&:hover": { bgcolor: "#eef2ff" } }}>Play</Button>
                                <Tooltip title="Download"><IconButton size="small" onClick={() => handleDownload(s.editFileUrl, s.editFileName || s.scriptId + "_edited")} sx={{ color: "#6b7280", "&:hover": { color: "#4f46e5", bgcolor: "#eef2ff" } }}><DownloadIcon sx={{ fontSize: 16 }} /></IconButton></Tooltip>
                              </Stack>
                            : <Typography sx={{ fontSize: "0.8rem", color: "#9ca3af" }}>—</Typography>
                          }
                        </TableCell>
                        ...
                        <TableCell sx={{ ...tdSx, maxWidth: 160 }}>
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            <Typography sx={{ fontSize: "0.85rem", color: "#6b7280", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{s.editComment || "—"}</Typography>
                            {s.editComment && <Tooltip title="View full"><IconButton size="small" onClick={() => setCommentModal({ open: true, text: s.editComment, title: `Edit Comment — ${s.scriptId}` })} sx={{ color: "#6b7280", flexShrink: 0, "&:hover": { color: "#4f46e5", bgcolor: "#eef2ff" } }}><ViewIcon sx={{ fontSize: 15 }} /></IconButton></Tooltip>}
                          </Stack>
                        </TableCell>
                        <TableCell sx={tdSx}><Typography sx={{ fontSize: "0.8rem", color: "#1f2937", whiteSpace: "nowrap" }}>{fmt(s.editDoneAt)}</Typography></TableCell>
                        <TableCell sx={tdSx}><Typography sx={{ fontSize: "0.85rem", color: "#059669", fontWeight: 500 }}>{s.editDoneBy || "—"}</Typography></TableCell>
                        <TableCell sx={tdSx}>
                          <Tooltip title="Update status / comment"><IconButton size="small" onClick={() => openDialog(s)} sx={{ color: "#6b7280", "&:hover": { color: "#ea580c", bgcolor: "#ffedd5" } }}><EditIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
                        </TableCell>
                      </TableRow>
      ...
      {/* ── Dialogs ── */}
      <UploadVideoDialog open={uploadOpen} onClose={() => { setUploadOpen(false); setUploadTarget(null); }} script={uploadTarget} onUploaded={load} showSnack={showSnack} />
      ...
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: "#ffffff", borderRadius: 2 } }}>
        <DialogTitle sx={{ color: "#111827", fontWeight: 700, borderBottom: "1px solid #e5e7eb", py: 2, px: 3, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <IconButton size="small" onClick={() => setViewOpen(false)} sx={{ color: "#6b7280" }}><CloseIcon fontSize="small" /></IconButton>
        </DialogTitle>
        <DialogContent sx={{ pt: 3, px: 3, pb: 4 }}>
          {viewScript && (
            <Box>
              <Stack direction="row" spacing={1.5} mb={2.5} alignItems="center"><ScriptId id={viewScript.scriptId} /><TypeBadge label={viewScript.scriptType} /></Stack>
              <Box sx={{ bgcolor: "#f8fafc", p: 2.5, borderRadius: 2, border: "1px solid #e5e7eb" }}>
                <Typography sx={{ fontSize: "0.95rem", color: "#374151", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{viewScript.scriptText}</Typography>
              </Box>
            </Box>
          )}
        </DialogContent>
      ...
      <Dialog open={dialogOpen} onClose={closeDialog} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: "#ffffff", borderRadius: 3 } }}>
        <DialogTitle sx={{ color: "#111827", fontWeight: 700, borderBottom: "1px solid #e5e7eb", pb: 2, display: "flex", alignItems: "center", gap: 1 }}>
          ...
          {dialogTarget && <Box component="span" sx={{ ml: 1, fontSize: "0.85rem", color: "#4f46e5", fontFamily: "monospace", fontWeight: 500 }}>{dialogTarget.scriptId}</Box>}
        </DialogTitle>
        <DialogContent sx={{ pt: 3, display: "flex", flexDirection: "column", gap: 3, mt: 1, overflowY: "auto", maxHeight: "70vh" }}>
          {dialogTarget && (
            <Box sx={{ bgcolor: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 1.5, p: 2 }}>
              <Stack direction="row" alignItems="center" gap={1} mb={1}><TypeBadge label={dialogTarget.scriptType} /><Typography sx={{ fontSize: "0.8rem", color: "#6b7280" }}>by {dialogTarget.createdBy}</Typography></Stack>
              <Typography sx={{ fontSize: "0.85rem", color: "#4b5563", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{dialogTarget.scriptText}</Typography>
              {dialogTarget.cutVideoUrl && dialogTarget.cutVideoUrl !== "pending" && (
                <Button size="small" onClick={() => openPlayer(dialogTarget.cutVideoUrl, 'Cut Video')} startIcon={<PlayIcon sx={{ fontSize: 16 }} />}
                  sx={{ mt: 1.5, color: "#4f46e5", textTransform: "none", fontWeight: 600, fontSize: "0.8rem", p: "4px 10px", border: "1px solid #c7d2fe", borderRadius: 1.5, "&:hover": { bgcolor: "#eef2ff" } }}>View Cut Video</Button>
              )}
            </Box>
          )}
          <Divider sx={{ borderColor: "#e5e7eb" }} />
          ...
          {needsReason && <TextField label="Reason *" multiline minRows={2} value={form.editHoldReason} onChange={(e) => setForm((f) => ({ ...f, editHoldReason: e.target.value }))} error={!!errors.editHoldReason} helperText={errors.editHoldReason} sx={inputSx} />}
          <Divider sx={{ borderColor: "#e5e7eb" }} />
          <TextField label="Comment" multiline minRows={3} placeholder="Notes for the team…" value={form.editComment} onChange={(e) => setForm((f) => ({ ...f, editComment: e.target.value }))} sx={inputSx} />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3, pt: 2, borderTop: "1px solid #e5e7eb", gap: 1 }}>
