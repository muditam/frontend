...
const BankYesCcTejasv = () => {
  ...
  const fetchData = async (pageArg = page, rowsArg = rowsPerPage) => {
    try {
      setLoading(true);
      const { data } = await api.get("/api/bank-reconciliation/yes-cc-tejasv", {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      setUploading(true);
      await api.post("/api/bank-reconciliation/yes-cc-tejasv/upload", formData, {
  ...
  const saveRowColor = async (id, color) => {
    await api.put(`/api/bank-reconciliation/yes-cc-tejasv/${id}`, {
      rowColor: color,
    });
  };
  ...
  return (
    <Box sx={{ p: 3, bgcolor: "#f5f7fb", minHeight: "100vh", boxSizing: "border-box" }}>
      ...
      >
        ...
        <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap">
          <Stack direction="row" spacing={1} alignItems="center">
            <Swatch
              color={LIGHT_GREEN}
              onClick={() => applyRowColor(LIGHT_GREEN)}
              title="Mark selected Green"
            />
            <Swatch
              color={LIGHT_RED}
              onClick={() => applyRowColor(LIGHT_RED)}
              title="Mark selected Red"
            />
            <Button
              variant="outlined"
      ...
      <Paper
        elevation={0}
        sx={{ mb: 2, px: 2, py: 2, borderRadius: 2, border: "1px solid #e0e3ef" }}
      >
                    ...
                    rows.map((row) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell sx={{ minWidth: 200 }}>{row.type}</TableCell>
                          <TableCell align="right">
                            {row.amount !== null && row.amount !== undefined ? formatNumber(row.amount) : ""}
                          </TableCell>
                          <TableCell sx={{ minWidth: 120 }}>{row.dr}</TableCell>
                          <TableCell sx={{ minWidth: 380 }}>{row.remarks}</TableCell>
                        </TableRow>
                      );
                    })
                  )}
