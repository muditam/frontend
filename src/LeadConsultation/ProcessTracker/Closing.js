...
const Closing = ({ presalesHba1c = "8", customerId }) => {
  ...
  const autoSaveClosing = (updatedFields) => {
    ...
    api
      .post("/api/consultation-details", {
        customerId,
        closing: updatedFields,
      })
      .catch((err) =>
        console.error(
          "Error auto-saving closing details:",
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((error) =>
        console.error(
          "Error fetching closing details:",
              ...
              {expectedResultsOptions.map((opt) => (
                <Grid item xs={12} key={opt.id}>
                  <Button
                    ...
                    sx={{
                      backgroundColor:
                        expectedResult === opt.id ? "black" : "inherit",
              ...
              {preferredDietOptions.map((diet) => (
                <Grid item xs={12} key={diet}>
                  <Button
                    ...
                    sx={{
                      backgroundColor:
                        preferredDiet === diet ? "black" : "inherit",
            ...
            <Grid container spacing={1}>
              {courseDurationOptions.map((duration) => (
                <Grid item xs={12} key={duration}>
                  <Button
                    variant={
                      courseDuration === duration ? "contained" : "outlined"
                    }
                    fullWidth
                    onClick={() => handleCourseDurationSelect(duration)}
                    sx={{
                      backgroundColor:
                        courseDuration === duration ? "black" : "inherit",
            ...
            <Grid container spacing={1}>
              {freebiesOptions.map((freebieOption) => (
                <Grid item xs={12} key={freebieOption}>
                  <Button
                    variant={
                      freebies.includes(freebieOption)
                        ? "contained"
                        : "outlined"
                    }
                    fullWidth
                    ...
                    sx={{
                      backgroundColor: freebies.includes(freebieOption)
                        ? "black"
                        : "inherit",
                      color: freebies.includes(freebieOption)
                        ? "white"
                        : "black",
                      fontSize: "0.8rem",
                    }}
          ...
          <Grid item xs={4}>
            ...
            <FormControl fullWidth size="small" sx={{ mb: 2 }}>
              ...
              <Select
                ...
                onChange={handleBloodTestSelect}
              >
